...
async def test_bstuokey_invisible_lock(hass: HomeAssistant) -> None:
    ...
    from custom_components.tuya_ble.devices import TuyaBLEDevice, get_device_product_info, TuyaBLECoordinator, TuyaBLEData
    ...
    device._device_info = TuyaBLEDeviceCredentials(
        ...
        functions=[
            {"code": "lock_motor_state", "dp_id": 47, "type": "Boolean", "values": None},
            {"code": "beep_volume", "dp_id": 31, "type": "Enum", "values": '{"range": ["mute", "low", "normal", "high"]}'},
            {"code": "language", "dp_id": 28, "type": "Enum", "values": '{"range": ["chinese_simplified", "english"]}'},
            {"code": "ble_unlock_check", "dp_id": 71, "type": "String", "values": None},
        ],
        status_range=[
            {"code": "alarm_lock", "dp_id": 21, "type": "Enum", "values": '{"range": ["wrong_finger", "wrong_password", "wrong_card"]}'},
    ...
    device.append_functions(device._device_info.functions, device._device_info.status_range)
    ...
    alarm_mapping = next(m for m in sensor_mappings if m.description.key == "alarm_lock")
    card_mapping = next(m for m in sensor_mappings if m.description.key == "unlock_card")
    ...
    device.datapoints._update_from_device(9, 0, 0, TuyaBLEDataPointType.DT_STRING, "medium")
    battery_sensor._handle_coordinator_update()
    assert battery_sensor.native_value == 60

    device.datapoints._update_from_device(9, 0, 0, TuyaBLEDataPointType.DT_STRING, "high")
    battery_sensor._handle_coordinator_update()
    ...
    beep_select_mapping = next(m for m in select_mappings if m.description.key == "beep_volume")
    lang_select_mapping = next(m for m in select_mappings if m.description.key == "language")

    beep_select = TuyaBLESelect(hass, coordinator, device, product_info, beep_select_mapping)
    ...
    device.datapoints._update_from_device(31, 0, 0, TuyaBLEDataPointType.DT_ENUM, 2) # normal
    ...
    unlock_check_mapping = next(m for m in button_mappings if m.description.key == "ble_unlock_check")

    unlock_button = TuyaBLEButton(hass, coordinator, device, product_info, unlock_check_mapping)
